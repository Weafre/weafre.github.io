---
layout: post
title: "Video Compression"
date: 2020-01-05
---

<figure>
 <img src='{{site.url}}/images/std.png' alt='Video coding standardization ' style="width:640px;height:400px;" class="center"/>
 <figcaption>
 	<center>
 Video coding standardization by <a href="https://www.slideshare.net/MathiasWien/trends-and-recent-developments-in-video-coding-standardization">RWTH AACHEN</a>
 </center>
 </figcaption>
</figure>
<ol>
<li>Introduction</li>
<p style="font-size:110%;">In this post, we are going through the key techniques using in video compresison algorithm such as (motion estimation, DCT transformation, quantization..). As you might know, the standards (MPEG-1,2,4; H264, H265(HEVC)) only defines the bitstream syntax and the
decoder behavior without defining the encoder. So, the principles of those techniques are used from the past standards to the state of the arts one.</p>

<li>Video coding principle</li>
<p style="font-size:110%;">
	
</p>


</ol>